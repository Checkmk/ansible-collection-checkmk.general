...
DOCUMENTATION = """
    ...
    short_description: Get a list of all BI rules of a BI pack

    description:
      - Returns a list of all BI rules of a BI pack.
...
- name: "Get all BI rules of every BI pack."
  ansible.builtin.debug:
    msg: "BI rules of pack {{ item.id }}: {{ lookup('checkmk.general.bi_rules', pack_id=item.id) }}"
...
RETURN = """
  _list:
    description:
      - A list of all BI rules of the BI pack.
    type: list
    elements: dict
"""
...
class LookupModule(LookupBase):
    def run(self, terms, variables, **kwargs):
        ...
        return [member_values(response, "rules")]
