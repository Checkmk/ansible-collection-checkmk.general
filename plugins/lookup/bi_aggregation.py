...
DOCUMENTATION = """
    ...
    description:
      - Returns the attributes of a BI aggregation in Checkmk.

    options:

      _terms:
        description:
          - One or more BI aggregation IDs, either as separate terms or as a single list.
        required: True
        type: list
        elements: str
...
EXAMPLES = """
- name: "Get the attributes of a BI aggregation."
  ansible.builtin.debug:
    msg: "Attributes of BI aggregation: {{ attributes }}"
  vars:
    attributes: "{{
...
- name: "Get BI aggregation attributes using inventory variables."
  ansible.builtin.debug:
    msg: "Attributes of BI aggregation: {{ attributes }}"
  vars:
    ...
    attributes: "{{ lookup('checkmk.general.bi_aggregation', 'aggr1') }}"
"""

RETURN = """
  _list:
    description:
      - A list of dicts of attributes of the BI aggregation(s).
    type: list
    elements: dict
"""
...
class LookupModule(LookupBase):
    def run(self, terms, variables, **kwargs):
        ...
        for term in self._flatten(terms):
            response = json.loads(api.get("/objects/bi_aggregation/" + term))
