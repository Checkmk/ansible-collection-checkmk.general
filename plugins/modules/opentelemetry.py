...
- C(activation) enables or disables the OpenTelemetry collector for a site.
  Since there is no read endpoint, C(changed) is always reported on success.
...
def _json_sort_key(v):
    return json.dumps(v, sort_keys=True)


def _is_desired_subset(desired, existing):
    """Return True if all desired key/values are present (and matching) in existing.

    For dicts: recurse, allowing extra keys in existing (API-added defaults).
    For lists: compare element-by-element using stable JSON serialisation after sorting.
    For scalars: direct equality.
    """
    if isinstance(desired, dict) and isinstance(existing, dict):
        return all(
            k in existing and _is_desired_subset(v, existing[k])
            for k, v in desired.items()
        )
    elif isinstance(desired, list) and isinstance(existing, list):
        return sorted(desired, key=_json_sort_key) == sorted(
            existing, key=_json_sort_key
        )
    else:
...
class OTelConfigAPI(CheckmkAPI):
    ...
    def _needs_update(self, existing):
        ...
        return not _is_desired_subset(desired, extensions)
...
def run_module():
    ...
    elif config_type in (_PROM_SCRAPE, _RECEIVER):
