...
- B(Note:) The read endpoint only exposes the enabled/disabled state, not the configured ports
  ...
  applies the configuration and reports C(changed) on every successful run.
...
# cannot be checked for idempotency.
...
class MetricBackendAPI(CheckmkAPI):
    ...
    def run(self):
        config = self.params.get("config") or {}
        has_extra = any(config.get(field) is not None for field in _EXTRA_CONFIG_FIELDS)

        # The read endpoint only exposes the enabled/disabled state, so we can
        # only skip the update when the state already matches and no ports or
