...
checkmk_oldstable="2.1.0p38"
checkmk_stable="2.2.0p19"
...
## Integration tests
find "${collection_dir}/tests/integration/targets/" -type f -name main.yml -exec sed -i "s/2.3.0.*/${checkmk_beta}\"/g" {} \; && echo "Updated Checkmk Stable version for integration tests to ${checkmk_beta}."
...
## Molecule tests
find "${collection_dir}/roles/" -type f -name all.yml -exec sed -i "s/2.3.0.*/${checkmk_beta}\"/g" {} \; && echo "Updated Checkmk Stable version for molecule tests to ${checkmk_beta}."
...
# Roles:
find "${collection_dir}/roles/" -type f -name main.yml -exec sed -i "s/2.3.0.*/${checkmk_beta}\"/g" {} \; && echo "Updated default Checkmk version for roles to ${checkmk_beta}."
...
find "${collection_dir}/roles/" -type f -name README.md -exec sed -i "s/2.3.0.*/${checkmk_beta}\"/g" {} \; && echo "Updated default Checkmk version in roles README to ${checkmk_beta}."
