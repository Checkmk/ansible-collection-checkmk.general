...
class TestProxyURLConstruction:
    """The URL building itself is covered by test_proxy.py, this only asserts
    that CheckmkAPI hands its parameters to the shared helper."""

    def test_no_proxy(self):
        api = CheckmkAPI(make_module())
        assert api.proxy_url is None

    def test_proxy_url_without_auth(self):
        api = CheckmkAPI(make_module({"proxy_url": "https://proxy.example.com:3128"}))
        assert api.proxy_url == "https://proxy.example.com:3128"
    ...
    def test_proxy_url_with_auth(self):
        ...
        assert api.proxy_url == "https://alice:s3cr3t@proxy.example.com:3128"

    def test_proxy_scheme_is_preserved(self):
        api = CheckmkAPI(make_module({"proxy_url": "http://proxy.example.com:3128"}))
        assert api.proxy_url == "http://proxy.example.com:3128"

    def test_proxy_user_without_pass_fails_the_module(self):
        module = make_module(
            {
                "proxy_url": "https://proxy.example.com:3128",
                "proxy_user": "alice",
        ...
        assert "must be provided together" in module.fail_json.call_args.kwargs["msg"]

    def test_proxy_pass_without_user_fails_the_module(self):
        module = make_module(
            {
                "proxy_url": "https://proxy.example.com:3128",
                "proxy_user": None,
    ...
    def test_invalid_scheme_fails_the_module(self):
        module = make_module({"proxy_url": "socks5://proxy.example.com:1080"})
        CheckmkAPI(module)
        module.fail_json.assert_called_once()
        assert "Unsupported scheme" in module.fail_json.call_args.kwargs["msg"]
...
class TestProxyEnvVars:
    ...
    def test_proxy_sets_env_vars_during_fetch(self):
        ...
        captured = {}
        self._run_fetch(api, captured)
        # The very same URL is exported for HTTP and HTTPS traffic: the scheme
        # says how to reach the proxy, not what it forwards.
        expected = "https://alice:s3cr3t@proxy.example.com:3128"
        assert captured["https_proxy"] == expected
        assert captured["http_proxy"] == expected
        assert captured["HTTPS_PROXY"] == expected
        assert captured["HTTP_PROXY"] == expected
