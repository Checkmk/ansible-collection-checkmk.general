...
- User module - The parameter ``auth_type`` now uses ``password`` as the default value. In most situations omitting the parameter was no problem at all, but it could fail in certain scenarios. Now the module will default to password authentication, if not defined otherwise.

Minor Changes
-------------

- User module - Checkmk 2.5.0 renames the ``mega_menu_icons`` parameter of the user management REST API endpoints. The new name ``main_menu_icons`` will be used by the user module for Checkmk 2.5.0, while the old name is used for earlier versions of Checkmk. The old name remains as an alias in Checkmk 2.5.0, so that it should still work with older versions of this collection for now.
...
Breaking in the new year with a breaking change.

Breaking Changes / Porting Guide
--------------------------------

- Lookup modules - Remove obsolete ``ANSIBLE_LOOKUP_`` environment variables and ``ansible_lookup_`` ansible variables. These were marked as obsolete already in ``5.11.0``. If you used this naming, use the new ``checkmk_var_`` naming instead, as the obsolete variable pattern is discouraged by the Ansible project.
- Rename authentication variables ``automation_user`` to ``api_user`` and ``automation_secret`` to ``api_secret``. This includes the internal variables ``checkmk_var_api_user`` and ``checkmk_var_api_secret`` as well. In all instances, ``automation_`` was renamed to ``api_``. Update your configuration accordingly. The old names ``automation_user`` and ``automation_secret`` are retained as aliases for a while. This is a temporary measure and is only supposed to ease the migration. Do not rely on these aliases!
